odoo.define('web.AbstractWebClient', function (require) {
    ...
    var GoTop = require('rainbow_community_theme.GoTop');
    ...
    var AbstractWebClient = Widget.extend(KeyboardNavigationMixin, {
        ...
        set_action_manager: function () {
            ...
            if (config.device.isMobile) {
                return this.action_manager.appendTo(fragment).then(function () {
                    ...
                    // dom.after(self.$('.o_main'), fragment, {
                    //     in_DOM: true,
                    //     callbacks: [{
                    //         widget: self.action_manager
                    //     }],
        ...
        set_go_top: function () {
            this.go_top = new GoTop(this);
            return this.go_top.appendTo(this.$el);
        },
        ...
        _onPushState: function (e) {
            this.do_push_state(_.extend(e.data.state, {
        ...
        _onWebClientStarted: function () {
            ...
            if (!this.isStarted) {
                ...
                core.bus.on('scroll_to_top', this.action_manager.$el.find("div.o_content"), this._scroll_to_top.bind(this));
        ...
        _scroll_to_top: function () {
            var offset = 300;
            var scrollingEl = this.action_manager.el.getElementsByClassName('o_content')[0];
            var $settingsEl = $(scrollingEl).find("div.settings");

            if (config.device.isMobile) {
                $(window).bind("touchend touchcancel touchleave", function (ev) {
                    if ($(this).scrollTop() > offset) {
                        core.bus.trigger('show_to_top');
                    } else {
                        core.bus.trigger('hide_to_top');
                    }
                });
            } else {
                if ($settingsEl.length > 0) {
                    $settingsEl.scroll(function () {
                        if ($(this).scrollTop() > offset) {
                            core.bus.trigger('show_to_top');
                        } else {
                            core.bus.trigger('hide_to_top');
